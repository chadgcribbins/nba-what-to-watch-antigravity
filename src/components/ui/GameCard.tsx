...
import FutureGameModal from './FutureGameModal';
...
export function GameCard({
    ...
    const handleRemind = (e: React.MouseEvent) => {
        ...
        if (!userEmail && !isReminded) {
            showToast(
                "🚨 DROP INTO THE LOCKER ROOM FIRST!",
                "error",
                {
                    label: "GO TO PROFILE",
        ...
        onToggleReminder();
        if (!isReminded) {
            showToast(`🎯 REMINDER SCHEDULED! We'll ping ${userEmail} when it's safe to drop in for ${game.awayTeam.abbreviation} @ ${game.homeTeam.abbreviation}.`, "arcade");
        }
    };
    ...
    return (
        <>
            ...
            >
                {/* Stretched Link for Primary Action (NBA.com) - only for non-scheduled games */}
                ...
                {userPick && (
                    <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 z-30 animate-in fade-in zoom-in duration-300">
                        <span className={clsx(
                            "text-[9px] font-black uppercase px-2 py-0.5 rounded-full border-2 border-black box-shadow-arcade-xs flex items-center gap-1 whitespace-nowrap",
                            isFinal ? "bg-arcade-green text-white" : "bg-arcade-blue text-white"
                        )}>
                            {isFinal ? (
                                <><span className="text-[10px]">🎯</span> Prediction Correct</>
                            ) : (
                                <><span className="text-[10px]">⚡</span> Target Locked</>
                            )}
                        </span>
                    </div>
                )}
